(() => {
  const key = 'fv_theme';
  const normalize = (value) => String(value || '').toLowerCase().endsWith('dark') ? 'dark' : 'light';
  ...
  function apply(value, persist = true) {
    ...
    document.querySelector('meta[name="theme-color"]')?.setAttribute('content', dark ? '#0b1220' : '#f4f7fb');
    document.querySelectorAll('[data-theme-toggle]').forEach((button) => {
      button.innerHTML = `${icon(dark)}<span>${dark ? 'Modo claro' : 'Modo escuro'}</span>`;
